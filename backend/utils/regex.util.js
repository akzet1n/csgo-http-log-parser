const REGEX = {
    ...
    KILL: /\d{2}\/\d{2}\/\d{4} - \d{2}:\d{2}:\d{2}.\d{3} - "([^<]*)<(?:\d*)><(.*)><(?:\w*)>" \[(?:.*)\] killed "([^<]*)<(?:\d*)><(.*)><(?:\w*)>" \[(?:.*)\] with (.*)/,
    CONNECTION: /\d{2}\/\d{2}\/\d{4} - \d{2}:\d{2}:\d{2}.\d{3} - "([^<]*)<(?:\d*)><(.*)><(?:\w*)>" ((entered)\w*|disconnected)/,
    WORLD: /\d{2}\/\d{2}\/\d{4} - \d{2}:\d{2}:\d{2}.\d{3} - World triggered "(\w*)"(?: \(CT "(\d*?)"\) \(T "(\d*?)"\))?/,
    SAY: /\d{2}\/\d{2}\/\d{4} - \d{2}:\d{2}:\d{2}.\d{3} - "([^<]*)<(?:\d*)><(.*)><(?:\w*)>" say "(.*)"/,
